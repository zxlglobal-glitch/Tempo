...
export default function Tempo() {
  ...
  useEffect(() => {
    if (!supabase || !user || !profileId || profileId === user.id) { setProfileBlocked(false); return; }
    let active=true;
    supabase.from('user_blocks').select('blocked_id').eq('blocker_id',user.id).eq('blocked_id',profileId).maybeSingle()
      .then(({data}) => { if(active) setProfileBlocked(Boolean(data)); });
    return () => { active=false; };
  }, [user?.id, profileId]);
