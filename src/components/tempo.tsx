...
export default function Tempo() {
  ...
  useEffect(() => { setCategory('Все'); setCategoryQuery(''); setLimit(20); }, [path]);
  useEffect(() => {
  ...
  const isAuth = path === '/login' || path === '/register'; const isForgot = path === '/forgot-password'; const isResend = path === '/resend-confirmation'; const isReset = path === '/reset-password';  const isEdit = path === '/profile/edit'; const isNew = path === '/workouts/new'; const isNotifications = path === '/notifications'; const isPeopleSearch = path === '/people';
  ...
  useEffect(() => {
    if (ready && supabase && !user && (isEdit || isNew || isNotifications || isMessages)) router.replace('/login');
  }, [ready, user, isEdit, isNew, isNotifications, isMessages, router]);
...
if (isReset) {
  ...
  const known = Boolean(workoutId) || path === '/' || isAuth || isForgot || isResend || isReset || isEdit || isNew || isNotifications || isMessages || isPeopleSearch || Boolean(profileId);
