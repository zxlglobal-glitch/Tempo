...
if (isReset) {
  ...
  return;
      ...
      if (isAuth) {
        ...
        if (isEdit) {
          ...
          const profile_theme = String(form.get('profile_theme') ?? 'none');
          if (!profileThemeIds.includes(profile_theme as (typeof profileThemes)[number]['id'])) throw new Error('Выберите фон профиля из предложенных вариантов.');
          const { error } = await supabase.from('profiles').update({ username, display_name: String(form.get('display_name')).trim(), city: String(form.get('city')).trim(), bio: String(form.get('bio')).trim(), profile_theme, avatar_path, ...(uploaded.length ? { avatar_url: avatarUrl({ ...profile, avatar_path } as Profile) } : {}) }).eq('id', current.id).select('id').single();
          if (error) throw error;
  ...
  {followListKind ? <FollowPeopleList kind={followListKind} people={followPeople} loading={followListLoading} error={followListError} profileName={displayName(profile)} /> : <><div className="feed-heading"><div><p className="eyebrow">{profileId ? 'ИСТОРИЯ ДВИЖЕНИЯ' : isSaved ? 'ВАША КОЛЛЕКЦИЯ' : 'ВДОХНОВЕНИЕ РЯДОМ'}</p><h2>{profileId ? 'Тренировки' : isSaved ? 'Сохранённые тренировки' : 'Лента сообщества'}</h2></div><span className="live-dot">Свежие сначала</span></div>{path === '/' && user && <div className="feed-tabs"><button className={feedMode === 'all' ? 'selected' : ''} onClick={() => { setFeedMode('all'); setLimit(20); }}>Все</button><button className={feedMode === 'following' ? 'selected' : ''} onClick={() => { setFeedMode('following'); setLimit(20); }}>Подписки</button></div>}<div className="category-toolbar">
    ...
    {(() => {
      ...
      const filteredCategories = allCategories.filter(c => c === 'Все' || c.toLowerCase().includes(query));
      const primaryCategories = allCategories.slice(0, 5);
      const extraCategories = allCategories.slice(5);
      const chooseCategory = (value: string) => { setCategory(value); setLimit(20); };
      if (query) {
        return <div className="filters category-filter-results" aria-label="Категории тренировок">
          {filteredCategories.map(c => <button aria-pressed={category === c} className={category === c ? 'selected' : ''} key={c} onClick={() => chooseCategory(c)}>{c}</button>)}
        </div>;
      }
      return <div className="category-filter-shell">
        <div className="filters category-primary-filters" aria-label="Основные категории тренировок">
          {primaryCategories.map(c => <button aria-pressed={category === c} className={category === c ? 'selected' : ''} key={c} onClick={() => chooseCategory(c)}>{c}</button>)}
          <button type="button" className={`category-expand-button ${categoriesExpanded ? 'open' : ''}`} aria-expanded={categoriesExpanded} aria-controls="extra-workout-categories" aria-label={categoriesExpanded ? 'Скрыть остальные категории' : 'Показать остальные категории'} onClick={() => setCategoriesExpanded(value => !value)}>
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 9 5 5 5-5"/></svg>
          </button>
        </div>
        <div id="extra-workout-categories" className={`category-extra-wrap ${categoriesExpanded ? 'open' : ''}`} aria-hidden={!categoriesExpanded}>
          <div className="filters category-extra-filters">
            {extraCategories.map(c => <button tabIndex={categoriesExpanded ? 0 : -1} aria-pressed={category === c} className={category === c ? 'selected' : ''} key={c} onClick={() => chooseCategory(c)}>{c}</button>)}
          </div>
        </div>
      </div>;
    })()}
...
function ProfileThemePicker({ defaultValue }: { defaultValue: string }) {
  const initial = profileThemeIds.includes(defaultValue as (typeof profileThemes)[number]['id']) ? defaultValue : 'none';
  const [value, setValue] = useState(initial);
  return <div className="profile-theme-field">
    <div className="profile-theme-field-head">
      <strong>Фон профиля</strong>
      <span>Выберите спортивный паттерн для карточки профиля</span>
    </div>
    <input type="hidden" name="profile_theme" value={value} />
    <div className="profile-theme-picker" role="radiogroup" aria-label="Фон профиля">
      {profileThemes.map(theme => <button
        key={theme.id}
        type="button"
        role="radio"
        aria-checked={value === theme.id}
        className={`profile-theme-option theme-preview-${theme.id} ${value === theme.id ? 'selected' : ''}`}
        onClick={() => setValue(theme.id)}
      ><span>{theme.label}</span><i aria-hidden="true" /></button>)}
    </div>
  </div>;
}
