  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (isAuth || isEdit || isNew || isNotifications || isMessages || workoutId || followListKind) return;
...
if (isReset) {
  ...
  const known = Boolean(workoutId) || path === '/' || isAuth || isForgot || isResend || isReset || isEdit || isNew || isNotifications || isMessages || Boolean(profileId);
