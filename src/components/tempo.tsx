...
export default function Tempo() {
  ...
  useEffect(() => { setCategory('Все'); setCategoryQuery(''); setLimit(20); setFeedMode('all'); }, [path]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (ready && supabase && !user && (isEdit || isNew || isNotifications || isMessages || isSaved)) router.replace('/login');
  }, [ready, user, isEdit, isNew, isNotifications, isMessages, isSaved, router]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (isAuth || isEdit || isNew || isNotifications || isMessages || isPeopleSearch || workoutId || followListKind) return;
