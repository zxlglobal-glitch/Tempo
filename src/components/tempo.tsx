...
export default function Tempo() {
  ...
  const isAuth = path === '/login' || path === '/register'; const isForgot = path === '/forgot-password'; const isReset = path === '/reset-password';  const isEdit = path === '/profile/edit'; const isNew = path === '/workouts/new';
  const profileId = path.startsWith('/people/') ? path.split('/')[2] : null;
  useEffect(() => {
  ...
  useEffect(() => {
    if (ready && supabase && !user && (isEdit || isNew)) router.replace('/login');
  }, [ready, user, isEdit, isNew, router]);
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (isAuth || isEdit || isNew || workoutId) return;
...
if (isReset) {
  ...
  const known = Boolean(workoutId) || path === '/' || isAuth || isForgot || isReset || isEdit || isNew || Boolean(profileId);
  return <div className="shell"><aside><Link className="logo" href="/">tempo<span>●</span></Link><p className="tagline">Движение объединяет</p><nav><Link className={path === '/' ? 'active' : ''} href="/">◉ <span>Лента тренировок</span></Link><Link href={user ? `/people/${user.id}` : '/login'}>◎ <span>Мой профиль</span></Link><div className="soon">↗ <span>Сообщения</span><small>Скоро</small></div></nav><div className="aside-bottom"><span className="mini-mark">↗</span><h3>В своём темпе.<br/>Вместе с другими.</h3><p>Каждая тренировка —<br/>уже шаг вперёд.</p></div></aside>
